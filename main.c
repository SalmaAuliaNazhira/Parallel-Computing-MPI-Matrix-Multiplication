...
void rotate_matrix(int *matrix, int n, int shift, MPI_Comm comm, int axis) {
    MPI_Cart_shift(comm, axis, shift, MPI_PROC_NULL, MPI_PROC_NULL);
    MPI_Sendrecv_replace(matrix, n * n, mpitype, MPI_PROC_NULL, 0, MPI_PROC_NULL, 0, comm, MPI_STATUS_IGNORE);
}
...
int main(int argc, char *argv[]) {
    ...
    partial_c_matrix = init_partial_c_matrix(n);

    // Cannon's Algorithm starts here
    for (int i = 0; i < p_sqrt; ++i) {
        ...
        rotate_matrix(sa, n, 1, comm, 1);
        rotate_matrix(sb, n, 1, comm, 0);
    }
